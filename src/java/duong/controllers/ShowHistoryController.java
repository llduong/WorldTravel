    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (session.getAttribute("USER") != null) {
                ...
                List<RegistrationDTTicket> listTicket = dao.getAllTicketOfUser(username);
                List<RegistrationDTTour> tours = new ArrayList<>();
                for (RegistrationDTTicket x : listTicket) {
                    tours.add(dao.findByPrimaryKey(x.getTourID()));
                }
                request.setAttribute("TOUR", tours);
                request.setAttribute("TICKET", listTicket);
            }
        } catch (Exception e) {
            log("ERROR at Show History Controller: " + e.getMessage());
        } finally {
