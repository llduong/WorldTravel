...
public class DeleteTourController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (count == 0) {
                result = dao.deleteTourByID(tourID);
            } else {
                result = false;
            }
            if (result) {
            ...
            log("ERROR Delete Tour Controller: " + e.getMessage());
        } finally {
            request.getRequestDispatcher("SearchTourController").forward(request, response);
        }
    }
